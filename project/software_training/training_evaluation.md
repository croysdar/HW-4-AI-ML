...
6. **Site-specific persistent FAR (night)** — The night BNN achieves ~74% per-frame FAR on hard-blank sequences (PIR-triggered night scenes where IR reflections resemble animal texture). Because the BNN classifies scene context rather than individual animals (7.5% bbox alignment from occlusion sensitivity), a site whose terrain or vegetation happens to produce animal-like IR patterns will trigger false alarms on *every* blank frame all night — not randomly, but persistently. The temporal filter does not help because the correlation is spatial (scene-level), not temporal. The ResNet-50 teacher achieves 0.0% per-sequence FAR on the same frames, confirming the failure is architectural capacity, not task difficulty. Per-site threshold calibration cannot rescue this: if blank frames already score 0.94–0.95 p(animal), a real animal may only score 0.96–0.98 — a margin indistinguishable from softmax noise. The model has lost *discriminability* at that site, not merely calibration. Only a higher-capacity model or site-specific retraining with labeled hard negatives from that location would resolve it.
