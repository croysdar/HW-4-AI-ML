...
| --------------------------------------------------- | ------------------------------------ | ------- | ------------ | ------------- | ------------ | ---------- |
| **Reg-file baseline** (`synth/config_regfile.json`) | 16,384-FF reg-file                   | 100 MHz | 1600×1600 µm | 215.3 mW      | ~12.5 (est.) | PASSED     |
| **1-SRAM** (`sram_1macro_experiment/`)              | 1× sky130_sram_1kbyte_1rw1r_32x256_8 | 20 MHz  | 2000×2000 µm | **2.91 mW**   | **0.50**     | PASSED     |
| **4-SRAM** (canonical `rtl/`, `synth/`, `tb/`)      | 4× sky130_sram_1kbyte_1rw1r_32x256_8 | 40 MHz  | 2400×1800 µm | **12.007 mW** | **2.5**      | bypassed†  |
...
Key results — **4-SRAM final design** (canonical `rtl/`, `synth/`, `tb/`, `sim/`):
...
### RTL (`rtl/`) — Final 4-SRAM Design

| File                                                                                             | Description                                                                                                      | Supports                        |
| ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| [rtl/top.sv](rtl/top.sv)                                                                         | **Final** `bnn_top`: 4×SRAM weight memory with 2-phase 128-bit reads, 3-stage pipelined `compute_core`, tile FSM | Checklist §2 RTL; Report §4, §5 |
| [rtl/compute_core.sv](rtl/compute_core.sv)                                                       | 3-stage BNN engine: XNOR → 8-chunk popcount → accumulate                                                         | Checklist §2 RTL; Report §4     |
| [rtl/interface.sv](rtl/interface.sv)                                                             | `axis_interface`: 2-deep AXI4-Stream skid buffer; breaks combinational ready path                                | Checklist §2 RTL; Report §5     |
| [rtl/sky130_sram_1kbyte_1rw1r_32x256_8_stub.v](rtl/sky130_sram_1kbyte_1rw1r_32x256_8_stub.v)     | Sky130 OpenRAM macro stub used for P&R (replaces behavioral wrapper post-synthesis)                              | Report §4 memory arch           |
| [rtl/sky130_sram_1kbyte_1rw1r_32x256_8_behav.sv](rtl/sky130_sram_1kbyte_1rw1r_32x256_8_behav.sv) | Behavioral SRAM model used by the 4-SRAM testbench                                                               | Report §6                       |
| [rtl/top_regfile.sv](rtl/top_regfile.sv)                                                         | _Register-file variant_ (640×256-bit reg-file, not the final design — kept as reference baseline)                | Report §7.1                     |
| [rtl/sram_behav_wrapper.sv](rtl/sram_behav_wrapper.sv)                                           | Behavioral SRAM wrapper used by the register-file variant's simulation-only flow                                 | Report §4                       |

### Testbench (`tb/`) — Final 4-SRAM Design

| File                                         | Description                                                                                                                | Supports                          |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| [tb/tb_top.sv](tb/tb_top.sv)                 | **Final** co-simulation against 4-SRAM `bnn_top`: AXI4-Stream streaming, `sv_dot` reference, conv2/3/4 tiles, backpressure | Checklist §2 testbench; Report §6 |
| [tb/tb_timing.sv](tb/tb_timing.sv)           | Timing testbench: per-tile cycle counts at 40 MHz — source of §8 throughput numbers                                        | Report §8.1                       |
| [tb/tb_top_regfile.sv](tb/tb_top_regfile.sv) | _Register-file variant_ testbench (not the final design)                                                                   | Report §7.1                       |

### Simulation Outputs (`sim/`) — Final 4-SRAM Design
...
| [sim/final_waveform.png](sim/final_waveform.png) | GTKWave screenshot: AXI4-Stream handshake + 2-phase SRAM read | Checklist §2 waveform; Report §6 |
| [sim/cosim_run.vcd](sim/cosim_run.vcd)           | VCD dump from co-simulation (source for final_waveform.png)   | Report §6                        |

### Synthesis (`synth/`) — Final 4-SRAM Design

| File                                                             | Description                                                                             | Supports                          |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------- | --------------------------------- |
| [synth/config.json](synth/config.json)                           | **Final** OpenLane 2 config: 4-SRAM `bnn_top`, Sky130A HD, 40 MHz, 2400×1800 µm         | Checklist §3 config               |
| [synth/macro_placement.cfg](synth/macro_placement.cfg)           | DEF macro placement: single-row, 4× sky130_sram_1kbyte_1rw1r_32x256_8                   | Report §7.4                       |
| [synth/openlane_run.log](synth/openlane_run.log)                 | OpenLane P&R log: 72/72 steps, DRC/LVS summary                                          | Checklist §3 run log; Report §7.4 |
| [synth/timing_report.txt](synth/timing_report.txt)               | OpenSTA setup+hold: WNS 0.0 ns / worst-case +11.42 ns at 40 MHz nom_tt                  | Checklist §3 timing; Report §7.4  |
| [synth/area_report.txt](synth/area_report.txt)                   | Cell + macro area: 4.32 mm² die, 4× SRAM macros + std-cell logic                        | Checklist §3 area; Report §7.4    |
| [synth/power_report.txt](synth/power_report.txt)                 | OpenSTA power: 12.007 mW total (TT 25°C 1.8V)                                           | Checklist §3 power; Report §7.4   |
| [synth/config_regfile.json](synth/config_regfile.json)           | _Register-file variant_ OpenLane 2 config: 100 MHz, 1600×1600 µm (not the final design) | Report §7.1                       |
| [synth/openlane_run_regfile.log](synth/openlane_run_regfile.log) | _Register-file variant_ OpenLane log: 78/78 steps, DRC/LVS PASSED                       | Report §7.1                       |

### SRAM Variant Directories (1-macro, 8-macro reference experiments)

The 1-SRAM and 8-SRAM variants are retained as reference experiments to bound the power–throughput tradeoff. The 4-SRAM design is the final and is now in the canonical `rtl/`, `tb/`, `sim/`, `synth/` directories above. See Report §4 (memory architecture) and §7.2–7.3 (synthesis results) for analysis.
...
```bash
cd project/m4
iverilog -g2012 -o sim/tb_top tb/tb_top.sv rtl/top.sv rtl/compute_core.sv rtl/interface.sv rtl/sky130_sram_1kbyte_1rw1r_32x256_8_behav.sv
